...
Vec2 FBmTerrain::getGradientNoise(const int x, const int z){
	static int seed = 666;
...
float FBmTerrain::getPerlinNoise(float x, float z){
	...
	float d1 = dot(v1, getGradientNoise(intX, intZ));
	float d2 = dot(v2, getGradientNoise(intX + 1, intZ));
	float d3 = dot(v3, getGradientNoise(intX, intZ + 1));
	float d4 = dot(v4, getGradientNoise(intX + 1, intZ + 1));
...
void FBmTerrain::update(){
	
}

void FBmTerrain::render() const {
	//glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	model->render();
}

float FBmTerrain::getFBmHeight(float x, float z){
	float total = 0;
	
	float f = 1.0f / (float)std::pow(2, 8);
	total += 30 * getPerlinNoise((x*f/4.0), (z*f/4.0));
	total += 15 * getPerlinNoise((x*f/2.0), (z*f/2.0));
	
	float total2 = 0;
	total2 += perlin.GetValue(x+0.5, 0, z+0.5);
	//total2 += getFBmHeightCustom(x+0.5, z+0.5);
	
	const int maxHeight = 5;
	//return total2 * maxHeight + total * maxHeight;
	return total * maxHeight;
}

float FBmTerrain::getFBmHeightCustom(float x, float z){         
	float amp = 1;
	float freq = frequency;
	
	float total = 0;
	for(int k = 0; k < octaves; ++k){
		float add = amp * getPerlinNoise(x*freq, z*freq);
		if(add > 0)
			add = -add + 1;
		else
			add = add + 1;
