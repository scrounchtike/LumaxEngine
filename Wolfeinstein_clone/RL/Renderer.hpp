...
public:
			...
			for(MaterialGroup3D* materialGroup : shaderGroup->materials){
				...
				for(GeometryGroup3D* geometryGroup : materialGroup->geometries){
					...
					if(!geometryGroup->isInstanced){
						...
						glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(float)*16, getCamera()->getProjectionMatrix().getHeadPointer());
						glBufferSubData(GL_UNIFORM_BUFFER, sizeof(float)*16, sizeof(float)*16, getCamera()->getViewMatrix().getHeadPointer());
	...
	static unsigned int uboWater;
	
	static std::map<std::string, unsigned> mapUBOs;
private:
	Camera* camera; // <- Same as lmx::camera
	Player* player;
};
