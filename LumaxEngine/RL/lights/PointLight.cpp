	...
	: position(position), color(color), attenuation(attenuation)
{
	...
	radius = (-attenuation.linear + sqrtf(attenuation.linear*attenuation.linear - 4.0f * attenuation.exponent * (attenuation.constant - (256.0f / 5.0f) * Imax))) / (2.0f * attenuation.exponent);
}
