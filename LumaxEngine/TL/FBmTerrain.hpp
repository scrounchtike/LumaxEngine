...
class FBmTerrain : public TerrainGenDesc
{
public:
	FBmTerrain(uint32 octaves = 4,
						 float persistence = 0.5f,
						 float lacunarity = 0.5f,
						 float max_height = 1.0f);

	float getHeight(float x, float z);
	Vec3 getNormal(float x, float z, float length_triangle);
	float getSlope(float x, float z);

	std::vector<float>* getHeights(int vertex_count, float xstart, float zstart);
	std::vector<Vec3>* getNormals(int vertex_count, float xstart, float zstart, float length_triangle);
	std::vector<float>* getSlopes(int vertex_count, float xstart, float zstart);
...
private:
	...
	Vec2 getGradientNoise(float x, float z);

	// Value Noise
	float getSmoothNoise(int x, int z);
	float getValueNoise(float x, float z);
	...
	float getHashNoise(int x, int z);
};
